...
class ArSysMultiBoards
{
	...
	public:
	    ...
	    ROS_INFO("ArSys node started with boards configuration: %s", boards_config.c_str());
            ...
            ROS_INFO("****************** Board Server Option **************************");
            ROS_INFO("* 1. ID                                                         *");
            ROS_INFO("* 2. Marker size                                                *");
            ROS_INFO("* 3. Corners: [x1,y1,z1],[x2,y2,z2],[x3,y3,z3],[x4,y4,z4]       *");
            ROS_INFO("* ATTENTION: Any space between points when you type the corners *");
            ROS_INFO("*****************************************************************");
        }
		      ...
		      ar_sys::Board_service::Response &res)
	{
            ...
            double size_mk = req.marker_size;
            int id_manual = req.id;
            string coordinates3d_manual = req.corners;


            ROS_INFO_STREAM("Sending to change parameters");
            change_parameters(id_manual , size_mk , coordinates3d_manual);


            ROS_INFO_STREAM("size_board2 " << boards.size() );
            res.result = true;

            return true;
	}

        void change_parameters(int id_manual , double size_mk , string corners_manual)
        {
            boards[0].uid = 1;
            boards[0].name = "Manually";
            boards[0].marker_size = size_mk;
            boards[0].config.updateBoard(id_manual, corners_manual);

            BoardConfiguration BConf=boards[0].config;

            ROS_INFO("\n ArSys node started with manual board configuration: \n");
	}
